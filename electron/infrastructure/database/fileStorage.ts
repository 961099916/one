...
async function ensureDir(dirPath: string): Promise<void> {
  if (!existsSync(dirPath)) {
    ...
    log.debug('[FileStorage] 创建目录:', dirPath)
  }
}
...
export async function initStorageDirs(): Promise<void> {
  ...
  log.info('[FileStorage] 存储目录初始化完成')
}
...
export async function saveFile(
  ...
  log.debug('[FileStorage] 保存文件:', relativePath)
  return relativePath
}
...
export async function saveImage(
  ...
  log.debug('[FileStorage] 保存图片:', relativePath)
  return relativePath
}
...
export async function saveBase64Image(
  ...
  const relativePath = path.join('images', filename)
  log.debug('[FileStorage] 保存 Base64 图片:', relativePath)
  return relativePath
}
...
export async function deleteFile(relativePath: string): Promise<void> {
  ...
  if (existsSync(absolutePath)) {
    ...
    log.debug('[FileStorage] 删除文件:', relativePath)
  }
}
